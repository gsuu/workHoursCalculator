...
test("승인된 평일 야간분을 넘겨 퇴근해도 야간근무 시간은 승인 분까지만 계산한다", () => {
  ...
  assert.equal(result.error, "");
  assert.equal(result.displayOvertimeMinutes, 210);
  assert.equal(result.displayNightMinutes, 0);
  assert.equal(result.weekday150, 210);
  assert.equal(result.weekday200, 0);
  assert.equal(result.leaveGrantMinutes, 315);
});

test("승인된 평일 연장분은 지각 시간으로 차감하지 않는다", () => {
  const result = calculateEntry({
    date: "2026-04-15",
    start: "09:03",
    end: "21:00",
...
test("승인 총시간 안에서는 실제 22시 이후 근무를 야간으로 다시 분리한다", () => {
  const result = calculateEntry({
    date: "2026-04-15",
    start: "08:22",
    end: "23:21",
    scheduledStartTime: "09:00",
    scheduledEndTime: "18:00",
    approvedOvertimeMinutes: 300,
    approvedNightMinutes: 60
  });

  assert.equal(result.error, "");
  assert.equal(result.displayOvertimeMinutes, 240);
  assert.equal(result.displayNightMinutes, 81);
  assert.equal(result.weekday150, 240);
